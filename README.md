# Moon Formation by Triple Phase Transition

**A theoretical framework proposing that the Moon may have emerged from a triple phase transition within the fully molten proto-Earth during early planetary differentiation.**

---

## Author

**Michel Debailleul**  
Geophysicist — Université libre de Bruxelles (ULB)  
ORCID: [0009-0003-1222-1433](https://orcid.org/0009-0003-1222-1433)  
Email: [michel.debailleul@yahoo.fr](mailto:michel.debailleul@yahoo.fr)

Preprint: [https://doi.org/10.6084/m9.figshare.32149192](https://doi.org/10.6084/m9.figshare.32149192)

---

## Abstract

The origin of the Moon remains one of the major unresolved questions in planetary science.

This work proposes an alternative framework to classical giant-impact models. It suggests that lunar formation may have resulted from a **triple phase transition** occurring within the differentiating proto-Earth.

The proposed mechanism couples three physical transitions:

- **Rheological transition** — formation of a coherent magmatic torus governed by Bingham–Herschel rheology.
- **Mechanical transition** — episodic coherent ejections once critical instability thresholds are crossed.
- **Magnetic transition** — progressive Fe–Ni segregation driving later terrestrial dynamo emergence.

A central proposition of the theory is that **progressive iron differentiation acts as the common physical driver** linking magma rheology, ejection dynamics, and long-term planetary magnetic evolution.

The model also makes a **falsifiable prediction**: the Moon may preserve an internal concentric stratigraphy recording successive ejection episodes.

---

## Validation pathway

The validation program is explicit:

1. **Lunar seismological observations**  
2. **3D SPH numerical simulations**

---

## Repository contents

| File | Description |
|------|-------------|
| `Moon Formation by Triple Phase Transition.pdf` | Full manuscript |
| `Double_Puits_Debailleul.pdf` | Key figure: double potential well and Kramers barrier |
| `La formation de la Lune.mp4` | Presentation video (French) |

---

## Citation

If you use this work, please cite as:

> Debailleul, M. (2026). *Moon Formation by Triple Phase Transition in the Differentiating Proto-Earth* [Preprint]. Figshare.  
> [https://doi.org/10.6084/m9.figshare.32149192](https://doi.org/10.6084/m9.figshare.32149192)

| `Moon Formation by Triple Phase Transition.pdf` | [Full manuscript](./Moon%20Formation%20by%20Triple%20Phase%20Transition.pdf) |
| `Double_Puits_Debailleul.pdf` | [Key figure](./Double_Puits_Debailleul.pdf) -

## License

This work is licensed under a **Creative Commons Attribution-NonCommercial 4.0 International License** (CC BY‑NC 4.0).  
© 2026 Michel Debailleul.

---

## Repository status

This repository contains the theoretical formulation of the model. Numerical validation and observational testing remain ongoing research directions.
